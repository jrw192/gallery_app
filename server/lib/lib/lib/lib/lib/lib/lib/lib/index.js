...
passport.deserializeUser(function (username, done) {
  userService.getUserByName(username).then(user => done(null, user)).catch(err => done(err));
});
...
app.get('/user/:id', (req, res) => {
  ...
  userService.getUserByName(id).then(response => {
    res.status(200).send(response);
  }).catch(err => {
    res.status(500).send(error);
  });
...
app.get('/images', async (req, res) => {
  s3Service.getAllImages().then(response => {
    res.status(200).send(response);
  }).catch(err => {
    res.status(500).send(err);
  });
...
app.post('/saveimage/:id', (req, res) => {
  const id = req.params.id;
  s3Service.saveImage(id, req.body).then(response => {
    res.status(200).send(response);
  }).catch(err => {
    res.status(500).send(err);
  });
...
app.post('/savepostcard/:id', (req, res) => {
  ...
  postcardService.savePostcard(postcard).then(response => {
    res.status(200).send(response);
  }).catch(err => {
    res.status(500).send(err);
  });
...
app.get('/getpostcards', (req, res) => {
  postcardService.getAllPostcards().then(response => {
    res.status(200).send(response);
  }).catch(err => {
    res.status(500).send(err);
  });
...
app.post('/savecity', (req, res) => {
  const body = req.body;
  locationService.saveCity(body).then(response => {
    res.status(200).send(response);
